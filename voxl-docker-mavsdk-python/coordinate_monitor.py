...
async def main():
    ...
    print("-- Format: x(m) | y(m) | z(m) | yaw(deg)")
    print("-- Saving coordinates to /statelog/coordinates.csv")
    print("-" * 60)

    # Ensure /statelog directory exists
    os.makedirs('/statelog', exist_ok=True)
    
    # Create CSV file with headers
    csv_file = '/statelog/coordinates.csv'
                ...
                if process.returncode == 0 and stdout:
                    ...
                    if len(stdout) >= MSG_SIZE:
                        try:
                            ...
                            print(f"Position: {x:8.3f} | {y:8.3f} | {z:8.3f} | {yaw:6.1f}°")
