...
def control_from_centroid(cx, cy, img_w=320, img_h=320, deadband=0.05,
                          k_yaw=(1.), k_vy=0.1,k_vz=0.6, forward_speed=0.5,
                          yaw_clip=20.0, vy_clip=0.3, vz_clip=0.3):
    if cx < 0 or cy < 0:
        return 0.0, 0.0, 0.0
    nx = (cx - img_w/2) / (img_w/2) # -1 to 1
    ny = (cy - img_h/2) / (img_h/2)
    #if abs(nx) <= deadband and abs(ny) <= deadband:
    #    return 0.5, 0.0, 0.0
    yaw_rate = float(np.clip(nx * k_yaw * 180/np.pi, -yaw_clip, yaw_clip)) if np.abs(nx) < 0.7 else 0.0
    print(round(nx, 1))
    vy = float(np.clip(nx * k_vy, -vy_clip, vy_clip)) if np.abs(nx) < 0.7 else 0.0
    vz = float(np.clip(ny * k_vz, -vz_clip, vz_clip))
    return 0.5, -vy, vz, yaw_rate
