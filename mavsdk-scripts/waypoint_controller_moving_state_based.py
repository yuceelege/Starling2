...
async def run():
    ...
    waypoints = [
        (0.0, 2.76, 0.0, 0.0)  # Relative to initial position: +2.76m in Y, same X/Z, same yaw
    ]
    

    # TARGET_SPACING = 0.022
    # SLEEP_TIME = 0.05
    # Calculate parameters for 0.25 m/s velocity
    TARGET_VELOCITY = 0.25  # m/s
    TARGET_SPACING = 0.022  # Keep same spacing for smooth trajectory
    SLEEP_TIME = TARGET_SPACING / TARGET_VELOCITY  # Calculate delay for 0.25 m/s
  
    
    # Setup CSV logging
    os.makedirs('/statelog', exist_ok=True)
    csv_file = '/statelog/coordinates_moving_10.csv'
    ...
    for rel_x, rel_y, rel_z, rel_yaw in waypoints:
        abs_x = start_x + rel_x
        abs_y = start_y + rel_y
        abs_z = start_z + rel_z
        abs_yaw = start_yaw + rel_yaw
        full_waypoints.append((abs_x, abs_y, abs_z, abs_yaw))
        print(f"Target waypoint: x={abs_x:.3f}, y={abs_y:.3f}, z={abs_z:.3f}, yaw={abs_yaw:.1f}°")
